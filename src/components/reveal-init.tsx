...
function refreshReveals() {
  ...
  const obs = new IntersectionObserver(
    (entries) => {
      for (const e of entries) {
        ...
        if (e.isIntersecting) {
          ...
          // Once visible, we can stop observing this specific element to save resources
          // if we don't want "re-arm" behavior.
          // obs.unobserve(el); 
        }
      }
    },
    {
      root: null,
      threshold: [0.05, 0.2], // Lower threshold to trigger "out" sooner when leaving
  ...
  // Store on window so subsequent refreshes can disconnect.
