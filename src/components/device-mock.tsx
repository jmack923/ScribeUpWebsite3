...
export function DeviceMock({ title = "Embedded Bill Control", bullets = [], compact = false }: DeviceMockProps) {
  ...
  return (
    <div ref={wrapRef} className={`relative mx-auto w-full ${compact ? "max-w-[352px]" : "max-w-[392px]"}`}>
      <div className="absolute inset-0 -z-10 rounded-[2.8rem] blur-3xl opacity-56 bg-[radial-gradient(circle_at_20%_10%,rgba(84,100,255,0.32),transparent_60%),radial-gradient(circle_at_80%_90%,rgba(4,173,255,0.22),transparent_60%)] animate-pulse-glow" />
      <Card
        shadow="none"
        radius="none"
              ...
              {bullets.map((b, i) => (
                <div
                  ...
                  className={`flex items-center justify-between rounded-xl border p-3 transition-all duration-300 ease-out ${
                    i === active
                      ? "border-primary/40 bg-primary-100/60 shadow-[0_10px_30px_rgba(84,100,255,0.14)]"
