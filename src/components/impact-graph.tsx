...
function MiniSpark({ points, accent = "rgba(84,100,255,0.85)" }: { points: number[]; accent?: string }) {
  const w = 104;
  const h = 28;
  const pad = 2;
  ...
  return (
    <svg viewBox={`0 0 ${w} ${h}`} className="h-[26px] w-[104px]" aria-hidden="true" preserveAspectRatio="none">
      <path d={d} fill="none" stroke={accent} strokeWidth="1.45" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
              ...
              return (
                ...
                >
                  ...
                  <div className="mt-2 h-[5px] rounded-full bg-slate-900/5 border border-slate-200/60 overflow-hidden">
                    <motion.div
                      className="h-full rounded-full bg-[linear-gradient(90deg,rgba(84,100,255,0.48),rgba(30,162,255,0.40))]"
